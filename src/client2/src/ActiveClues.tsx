...
type ActiveCluesProps = {
  data: m.ClientGameState;
};

export default function ActiveClues(props: ActiveCluesProps) {
  const classes = useStyles();

  const { clues, myVote, players } = props.data;
  const winningPlayer = _getWinningPlayer(props.data);
  const rows = [];
  for (const player of Object.keys(clues)) {
    const row = clueToRow(player, clues[player], props.data);
